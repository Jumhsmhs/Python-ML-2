...
class KMeans:
    ...
    def init_centroids(self):
        ...
        for i in range(self.n_clusters):
            rand_index = np.random.choice(range(len(self.fit_date)))
            self.init_centroids.append(self.fit_data[rand_index])
        ...
        def fit(self, fit_data, fit_labels):
            ...
            while not self.convergod(self.iterations, old_centroids, self.centroids):
                ...
                for j, sample in tqdm(enumerate)(self.fit_data):
                    ...
                    def update_centroids(self):
                        ...
                        for i in range(self.n_clusters):
                            ...
                            if cluster == []:
                                ...
                                self.centeroids[i] = np.mean(np.vstack((self.centeroids[i], cluster)), axis=0)
                    
                    def reshape_cluster(self):
                        for id, mat in list(self.cluster['data'].items()):
                    ...
                    def converged(self, iterations, centroids, update_centroids):
                        ...
                        self.centeroids_dist = np.linalg.norm(
                            np.array(update_centroids)-np,array(centeroids))

                        if self.centeroids_dist <= 1e-10:
